...
```bash
INIT=$(node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```

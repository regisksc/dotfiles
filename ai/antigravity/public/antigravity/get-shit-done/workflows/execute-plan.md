...
```bash
node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
```bash
node ~/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
